...
const MyInterests = () => {
    ...
    const handlePay = async (listingId: string) => {
      try {
                      ...
                      {interestedUsers.map((user) => (
                        <tr key={user?.id} className={`${userState?.confirmed_deal ? 'bg-gray-100' : undefined}`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                            {user?.make_deal && !user?.confirm_deal && (
                              <>
                                {!user?.deposit_paid && <button
                                  type="button"
                                  onClick={() => handlePay(user?.listing)} // Pass the listingId to the function
                                  disabled={isButtonDisabled || isRequestPending || userState?.confirmed_deal}
                                  className={`mr-2 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 ${
                                    isButtonDisabled || isRequestPending || userState?.confirmed_deal
